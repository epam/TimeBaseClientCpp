...
#include "describe_stream_request.h"
...
bool DescribeStreamRequest::describe(std::string &ddl) {
    ddl.clear();
    ...
    if (!nameEquals(rootElement, "describeStreamResponse")) {
        return false;
    }

    XMLElement *spacesElement = rootElement->FirstChildElement("ddl");

    if (NULL != spacesElement) {
        ddl.append(spacesElement->GetText());
        return true;
    }
